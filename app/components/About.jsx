  ...
  return (
   ...
   >
    ...
    className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
        <motion.div className='w-64 sm:w-70 rounded-3xl max-w-none'
     ...
     className='flex-1'>
      <p className='mb-10 max-w-2xl font-Ovo text-justify'>Je suis une développeuse web passionnée, avec un intérêt particulier pour le développement front-end et le design UI/UX. Mon parcours m'a permis de développer une solide expertise dans la création de sites web et d'applications performants, en mettant un accent particulier sur l'expérience utilisateur.</p>
        ...
        className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl '>
           {infoList.map(({icon, icodrak, title, description}, index)=>(
            ...
            className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer  hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black  dark:border-red dark:hover:shadow-red dark:hover:bg-black' key={index}>
                 <Image src={isDarkMode ? icodrak : icon} alt={title} className='w-7 mt-3' />
